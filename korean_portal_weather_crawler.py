...
NAVER_SELECTORS = {
    "location": "strong.location_name, span.location_name",
    "temperature": ".card_data_now .card_now_temperature",
    "status": ".card_data_detail .card_date_emphasis",
    "temperature2": ".card_now_temperature",
    "status2": ".card_date_emphasis",
}
...
def _safe_get_text_multi(
    ...
    """
    Try multiple selectors one by one until one succeeds.

    Args:
        driver: Selenium WebDriver instance.
        selectors: List of (By, selector) tuples to try in order.
        timeout: Maximum wait time in seconds for each selector.
        field_name: Optional field name for logging.

    Returns:
        First non-None text found, or None if all selectors fail.
    """
...
def scrape_weather(
    ...
    try:
        ...
        location = _safe_get_text(
            driver,
            By.CSS_SELECTOR,
            NAVER_SELECTORS["location"],
            timeout=timeout,
            field_name="location",
        )

        temperature = _safe_get_text_multi(
            driver,
            selectors=[
                (By.CSS_SELECTOR, NAVER_SELECTORS["temperature"]),
                (By.CSS_SELECTOR, NAVER_SELECTORS["temperature2"]),
            ],
            timeout=timeout,
            field_name="temperature",
        )

        status = _safe_get_text_multi(
            driver,
            selectors=[
                (By.CSS_SELECTOR, NAVER_SELECTORS["status"]),
                (By.CSS_SELECTOR, NAVER_SELECTORS["status2"]),
            ],
            timeout=timeout,
            field_name="status",
        )
...
def run_crawler(
    ...
    Note:
        Logging is configured only once per process. Subsequent calls to
        run_crawler() will reuse the existing logging configuration.
...
def main() -> None:
    ...
    if not any([data.location, data.temperature, data.status]):
        logger.error("No weather data scraped. The page structure may have changed.")
        return
